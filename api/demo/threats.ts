...
async function rateLimitedRequest(url: string, headers: any) {
  const now = Date.now()
...
function getThreatLevel(stats: any): string {
...
function getThreatTitle(threat: any): string {
...
function getDetectionSource(threat: any): string {
  const attributes = threat.attributes || {}
  const stats = attributes.last_analysis_stats || {}
...
function getThreatType(threat: any): string {
  const attributes = threat.attributes || {}
...
function getThreatStatus(threat: any): string {
  const attributes = threat.attributes || {}
  const stats = attributes.last_analysis_stats || {}
  const malicious = stats.malicious || 0
  ...
  try {
    ...
    try {
      ...
      if (response.data && response.data.data && response.data.data.length > 0) {
        ...
        threats = realThreats.map((threat: any) => ({
    ...
    threats.sort((a, b) => {
      const levelOrder: any = { 'critical': 4, 'high': 3, 'medium': 2, 'low': 1 }
